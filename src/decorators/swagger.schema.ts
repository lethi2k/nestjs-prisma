...
function explore(instance: Object, propertyKey: string | symbol) {
  ...
  const parametersWithType = _.mapValues(
    ...
    (param) => ({
      type: types[param.index] as Type<unknown>,  // Ensure type is correctly asserted
      name: param.data,
      required: true,
    }),
  );
  ...
  for (const [key, value] of Object.entries(parametersWithType)) {
    ...
    if (Number(keyPair[0]) === RouteParamtypes.BODY) {
      // Ensure value.type is correctly typed
      return (value as { type: Type<unknown> }).type as unknown as string;
    }
  }
...
function RegisterModels(): MethodDecorator {
  return (target, propertyKey, descriptor: PropertyDescriptor) => {
    ...
    // Ensure body is a valid Type<unknown> before passing to ApiExtraModels
    return body && typeof body === 'function' ? ApiExtraModels(body)(target, propertyKey, descriptor) : descriptor;
  };
}
